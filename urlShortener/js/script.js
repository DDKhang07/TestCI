...
parseEmojis();});$(document).on('click','.clear-btn',function(){$('#result').slideUp();wipeHistory();$('#result').removeClass('has-content');$('#result').html('');$('#result').fadeIn();displayClearBtn();});$(document).bind('DOMSubtreeModified','#nhvc_inputbox_case',function(){if($('#nhvc_wjfo9i32 #nhvc_inputbox_case').hasClass('worked')){setCookie('captcha_completed','yes',90);$.modal.close();}});function displayClearBtn(){if(!$('#result').is(':empty')){$('.clear-btn').slideDown();}else{$('.clear-btn').slideUp();}}
...
function parseEmojis(){twemoji.parse(document.body,{folder:'svg',ext:'.svg'});}
